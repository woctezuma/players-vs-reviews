...
def rank_games_based_on_ratio_players_vs_reviews(steamspy_data, ratio_exponent=1):
    ...
    ranking = zip(appID_ranking, game_name_ranking)
...
def print_ranking_to_file_stream(ranking, outfile=None, num_top_games_to_print=None, width=40):
    ...
    # The following line allows to copy the ranking, to compute its length before iterating over it.
    ranking_list = list(ranking)

    num_games = len(ranking_list)
    ...
    for (iter_no, game_info) in enumerate(ranking_list):
